...
import dev.openallay.context.game.ObservableGameStateSnapshot.ModsState;
import dev.openallay.context.game.ObservableGameStateSnapshot.OptionValue;
import dev.openallay.context.game.ObservableGameStateSnapshot.OptionsState;
import dev.openallay.context.game.ObservableGameStateSnapshot.PacksState;
...
public final class InspectGameStateTool
    ...
    private static final ToolDescriptor<Input, Output> DESCRIPTOR = new ToolDescriptor<>(
            ...
            "Inspect one registered section of directly player-observable game/client state. "
                    + "Current biome/coordinates/dimension/direction are DIAGNOSTICS category:position, never WORLD_QUERY. "
    ...
    public ToolResult<Output> invoke(ToolInvocationContext context, Input input) {
        ...
        String normalizedQuery = normalize(input.query());
        if (input.section() == GameStateSection.WORLD_QUERY
                && isClientPositionAlias(normalizedQuery)) {
            return failure(
                    "wrong_section",
                    "Current biome, coordinates, dimension, and direction are client diagnostics; call DIAGNOSTICS with query category:position");
        }
        if (input.section() == GameStateSection.WORLD_QUERY
        ...
        try {
            return new ToolResult.Success<>(select(snapshot, input.section(), normalizedQuery));
        } catch (IllegalArgumentException invalid) {
            return failure("invalid_tool_arguments", invalid.getMessage());
        }
    }

    private static boolean isClientPositionAlias(String query) {
        return Set.of(
                        "biome", "position", "coordinates", "coordinate", "location",
                        "dimension", "direction", "yaw", "pitch",
                        "生物群系", "坐标", "位置", "维度", "方向")
