...
| State | In-memory multi-session conversations; one active request per session; endpoint-scoped 429 queueing |
...
Shared server inference and client-local inference use the same endpoint rate
scheduler. TomeWisp imposes no default global concurrency count. Different
sessions may run concurrently; a provider HTTP 429 requeues the request and
closes only that endpoint's dispatch gate for `Retry-After`, or a cancellable
exponential backoff if absent. Scheduling rotates sessions fairly when dispatch
resumes. Queue position/start events are visible. Cancellation or disconnect
removes queued work. Context and session history are captured at execution time.
Phase 2 imposes no queue-count cap.
...
the latest answer in memory. They are keyed by actor identity plus `sessionId`;
one actor can own and concurrently use multiple isolated sessions. They are
cleared on disconnect, explicit close/clear, and shutdown.
They are not permanent memory and never override live game facts.

One active request is allowed per session. New work in that same session returns
`agent_busy`; other sessions remain independent in client or server mode. Model
requests pass through the endpoint-scoped 429 scheduler described in section
5.3. Cancellation is cooperative across HTTP, SSE, local tools, remote
correlations, and queued requests. Late callbacks check request identity before
publishing.
